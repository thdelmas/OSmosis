"""ROM finder — search for available ROMs across multiple sources."""
...
def api_romfinder(codename):
    ...
    for name in [codename, model.lower()] if model else [codename]:
        ...
        try:
            ...
            if (
                r.returncode == 0
                and "Not Found" not in r.stdout
                and name in r.stdout.lower()
            ):
                ...
                results.append(
                    {
                        ...
                        "version": version,
                        "filename": latest,
                        "download_url": f"https://images.ecloud.global/stable/{name}/{latest}"
                        if latest
                        else "",
        ...
        try:
            r = subprocess.run(
                [
                    ...
                    "--max-time",
                    "6",
                    f"https://twrp.me/Devices/{twrp_brand}/",
                ],
                capture_output=True,
                text=True,
                timeout=10,
    ...
    try:
        ...
        for channel in ["edge", "v24.12", "v24.06", "v23.12"]:
            ...
            try:
                ...
                if (
                    r.returncode == 0
                    and "404" not in r.stdout[:200]
                    and pmos_device in r.stdout.lower()
                ):
            ...
            for pmos_term in pmos_terms:
                ...
                if r.returncode == 0 and r.stdout.strip():
                    try:
                        ...
                        for sr in search_data.get("query", {}).get(
                            "search", []
                        ):
                            title = sr.get("title", "")
                            if codename in title.lower() or (
                                model and model.lower() in title.lower()
                            ):
                                results.append(
                                    {
                                        "id": "postmarketos",
    ...
    search_links = [
        ...
        },
        {
            "id": "lineageos_wiki",
            "name": "LineageOS Wiki",
            "url": "https://wiki.lineageos.org/devices/#samsung",
        },
        {
    ...
    for _key, entry in index.items():
        if entry.get("codename") == codename:
            matched = False
            for r in results:
                if r["id"] == entry.get("rom_id") and r.get(
                    "filename"
                ) == entry.get("filename"):
